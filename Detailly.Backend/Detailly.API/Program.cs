...
public partial class Program
{
    private static async Task Main(string[] args)
    {
        ...
        try
        {
            ...
            Log.Information("Detailly API started successfully.");
            app.Run();
        }
        catch (HostAbortedException)
        {
            // EF Core tools abort the host after they take DbContext.
            // This is not a Runtime Error – just leave silently.
            Log.Information("Host aborted by EF Core tooling (design-time) - its ok.");
        }
        catch (Exception ex)
        {
            // Any startup failure will be logged here
            Log.Fatal(ex, "Detailly API terminated unexpectedly.");
        }
        finally
        {
